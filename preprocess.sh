...
# python scripts/make_udic.py --tokens 1000 --out $DIR_DIC/$FILE_UDIC_CSV
# /usr/local/libexec/mecab/mecab-dict-index -d /usr/local/lib/mecab/dic/ipadic -u $DIR_DIC/$FILE_UDIC -f utf-8 -t utf-8 $DIR_DIC/$FILE_UDIC_CSV
python scripts/tokenize_wikipedia.py --extract $DIR_WORK/$FILE_EXTRACT_WITH_HYPERLINKS --udic $DIR_DIC/$FILE_UDIC --out $DIR_WORK/$FILE_CORPUS
